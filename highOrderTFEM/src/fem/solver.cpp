...
void Solver::simulate_steps(int n_steps){
    for(int i = 0; i < n_steps; (i++, n_total_steps++)){
        ...
        // Manifest individual variables needed for capture-by-copy
        auto mesh = this->mesh;
        auto point_mass_inv_readonly = this->point_mass_inv_readonly;
        auto prev_points_readonly = this->prev_point_weights_readonly;
        ...
        Kokkos::parallel_for(mesh.edge_count(), KOKKOS_LAMBDA(int i){
            ...
            double contribution_to_1 = -k * dt * point_mass_inv_readonly(e[0]) * A_12 * prev_points_readonly(e[1]);
            double contribution_to_2 = -k * dt * point_mass_inv_readonly(e[1]) * A_21 * prev_points_readonly(e[0]);
